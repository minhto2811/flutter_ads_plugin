...
class FlutterAdsAppOpen {
  ...
  static Duration _maxCacheDuration = const Duration(hours: 4); // Ad cache time limit
  static DateTime _appOpenLoadTime = DateTime.now();            // Time when ad was last loaded
  ...
  static FutureOr<void> showAdIfAvailable() async {
    ...
    final isExpired = DateTime.now().subtract(_maxCacheDuration).isAfter(_appOpenLoadTime);
    if (isExpired) {
      _appOpenAd!.dispose();
      _appOpenAd = null;
